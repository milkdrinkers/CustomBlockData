...
final class BlockDataListener implements Listener {
    ...
    */
    ...
    /**
     * BlockPhysicsEvent is used for essentially the same thing as BlockBreakEvent but more precise
     */
    @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
    public void onPhysics(BlockPhysicsEvent event) {
        // event.getBlock().getType() returns air when the block is broken
        if (event.getBlock().getType().equals(Material.AIR) && CustomBlockData.hasCustomBlockData(event.getBlock(), plugin)) {
